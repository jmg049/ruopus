...
fn ffmpeg_file_duration_and_packets() {
    ...
    while let Some(pkt) = reader.next() {
        let parsed = ruopus::Packet::parse(&pkt.data).expect("valid Opus packet");
...
fn ogg_opus_write_read_round_trip() {
    ...
    let mut tags = OpusTags {
        vendor: b"ruopus test".to_vec(),
        comments: Vec::new(),
    };
    tags.push("TITLE", "Round Trip");
    tags.push("artist", "ruopus");
    ...
    assert_eq!(reader.tags().get("ARTIST").as_deref(), Some("ruopus"));
...
fn opus_head_round_trips_all_families() {
    ...
    let surround = OpusHead {
        ...
        channel_mapping: ruopus::ogg::ChannelMapping::Table {
            family: 1,
            stream_count: 4,
            coupled_count: 2,
...
fn decodes_a_real_ogg_opus_file_end_to_end() {
    ...
    let (pcm, head) = ruopus::decode_ogg_opus(&data).expect("decode");
