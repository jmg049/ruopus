//! Packet-loss-concealment kernels (normative `pitch.c`, `celt_lpc.c`,
//! float build): pitch search over the synthesis history, linear
//! prediction, and the FIR/IIR filters the extrapolation runs in.
//!
...
mod tests {
    ...
    /// Pins generated by compiling the reference float `pitch.c` /
    /// `celt_lpc.c` and running them over this exact synthetic signal.
    #[test]
